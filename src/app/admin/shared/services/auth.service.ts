...
export class AuthService {
  ...
  get token(): string | null {
    const fbTokenExp: string | null = localStorage.getItem('fb-token-exp');
    if (fbTokenExp) {
      let expDate: Date = new Date(fbTokenExp)
    ...
    return this.http.post<User>(`https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=${environment.apiKey}`, user)
      .pipe(
        tap((newUser: User) => this.setToken({idToken: newUser.idToken!, expiresIn: newUser.expiresIn!})),
